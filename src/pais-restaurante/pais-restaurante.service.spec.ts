...
describe('PaisRestauranteService', () => {
  ...
  it('addRestaurantePais should add a restaurante to a pais', async () => {
    ...
    const result: PaisEntity = await service.addRestaurantePais(newPais.id, newRestaurante.id);
  ...
  it('addRestaurantePais should thrown exception for an invalid restaurante', async () => {
    ...
    await expect(() => service.addRestaurantePais(newPais.id, "0")).rejects.toHaveProperty("message", "The restaurante with the given id was not found");
  });
  ...
  it('addRestaurantePais should throw an exception for an invalid pais', async () => {
    ...
    await expect(() => service.addRestaurantePais("0", newRestaurante.id)).rejects.toHaveProperty("message", "The pais with the given id was not found");
  });
  ...
  it('findRestauranteByPaisIdRestauranteId should return restaurante by pais', async () => {
    ...
    const storedRestaurante: RestauranteEntity = await service.findRestauranteByPaisIdRestauranteId(pais.id, restaurante.id, )
  ...
  it('findRestauranteByPaisIdRestauranteId should throw an exception for an invalid restaurante', async () => {
    await expect(()=> service.findRestauranteByPaisIdRestauranteId(pais.id, "0")).rejects.toHaveProperty("message", "The restaurante with the given id was not found"); 
  });

  it('findRestauranteByPaisIdRestauranteId should throw an exception for an invalid pais', async () => {
    const restaurante: RestauranteEntity = restaurantesList[0]; 
    await expect(()=> service.findRestauranteByPaisIdRestauranteId("0", restaurante.id)).rejects.toHaveProperty("message", "The pais with the given id was not found"); 
  });
  ...
  it('findRestauranteByPaisIdRestauranteId should throw an exception for an restaurante not associated to the pais', async () => {
    ...
    await expect(()=> service.findRestauranteByPaisIdRestauranteId(pais.id, newRestaurante.id)).rejects.toHaveProperty("message", "The restaurante with the given id is not associated to the pais"); 
  });

  it('findRestaurantesByPaisId should return restaurantes by pais', async ()=>{
    const restaurantes: RestauranteEntity[] = await service.findRestaurantesByPaisId(pais.id);
    expect(restaurantes.length).toBe(5)
  });

  it('findRestaurantesByPaisId should throw an exception for an invalid pais', async () => {
    await expect(()=> service.findRestaurantesByPaisId("0")).rejects.toHaveProperty("message", "The pais with the given id was not found"); 
  });
  ...
  it('associateRestaurantesPais should update restaurantes list for a pais', async () => {
    ...
    const updatedPais: PaisEntity = await service.associateRestaurantesPais(pais.id, [newRestaurante]);
    ...
    expect(updatedPais.restaurantes[0].estrellas).toBe(newRestaurante.estrellas);
  ...
  it('associateRestaurantesPais should throw an exception for an invalid pais', async () => {
    ...
    await expect(()=> service.associateRestaurantesPais("0", [newRestaurante])).rejects.toHaveProperty("message", "The pais with the given id was not found"); 
  });
  ...
  it('associateRestaurantesPais should throw an exception for an invalid restaurante', async () => {
    ...
    await expect(()=> service.associateRestaurantesPais(pais.id, [newRestaurante])).rejects.toHaveProperty("message", "The restaurante with the given id was not found"); 
  });
  ...
  it('deleteRestauranteToPais should remove an restaurante from a pais', async () => {
    ...
    const storedPais: PaisEntity = await paisRepository.findOne({where: {id: pais.id}, relations: ["restaurantes"]});
    const deletedRestaurante: RestauranteEntity = storedPais.restaurantes.find(a => a.id === restaurante.id);
  ...
  it('deleteRestauranteToPais should thrown an exception for an invalid restaurante', async () => {
    await expect(()=> service.deleteRestaurantePais(pais.id, "0")).rejects.toHaveProperty("message", "The restaurante with the given id was not found"); 
  });

  it('deleteRestauranteToPais should thrown an exception for an invalid pais', async () => {
    const restaurante: RestauranteEntity = restaurantesList[0];
    await expect(()=> service.deleteRestaurantePais("0", restaurante.id)).rejects.toHaveProperty("message", "The pais with the given id was not found"); 
  });
  ...
  it('deleteRestauranteToPais should thrown an exception for an non asocciated restaurante', async () => {
    ...
    await expect(()=> service.deleteRestaurantePais(pais.id, newRestaurante.id)).rejects.toHaveProperty("message", "The restaurante with the given id is not associated to the pais"); 
  }); 
