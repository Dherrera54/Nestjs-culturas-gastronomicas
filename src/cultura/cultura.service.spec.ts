...
describe('CulturaService', () => {
 ...
 it('findOne should throw an exception for an invalid culture', async () => {
   await expect(() => service.findOne("0")).rejects.toHaveProperty("message", "La cultura gastronomica con id dado no se encontró")
 });
 ...
 it('create should return a new culture', async () => {
   ...
   const storedCultura: CulturaEntity = await repository.findOne({where: {id: newCultura.id}})
 ...
 it('update should modify a culture', async () => {
   ...
   cultura.descripcion = "New description";
    const updatedCulture: CulturaEntity = await service.update(cultura.id, cultura);
   expect(updatedCulture).not.toBeNull();
    const storedCultura: CulturaEntity = await repository.findOne({ where: { id: cultura.id } })
   expect(storedCultura).not.toBeNull();
 ...
 it('update should throw an exception for an invalid culture', async () => {
   ...
   cultura = {
     ...cultura, nombre: "New name", descripcion: "New description"
   }
   await expect(() => service.update("0", cultura)).rejects.toHaveProperty("message", "La cultura gastronomica con id dado no se encontró")
 });
 ...
 it('delete should remove a culture', async () => {
   ...
   await service.delete(cultura.id);
    const deletedCulture: CulturaEntity = await repository.findOne({ where: { id: cultura.id } })
 ...
 it('delete should throw an exception for an invalid culture', async () => {
   const cultura: CulturaEntity = culturaList[0];
   await service.delete(cultura.id);
   await expect(() => service.delete("0")).rejects.toHaveProperty("message", "La cultura gastronomica con id dado no se encontró")
 });
