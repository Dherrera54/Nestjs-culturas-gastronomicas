...
describe('PaisService', () => {
 ...
 let paisesList: PaisEntity[];
  ...
  it('findOne should throw an exception for an invalid pais', async () => {
    await expect(() => service.findOne("0")).rejects.toHaveProperty("message", "The pais with the given id was not found")
  });
  ...
  it('update should throw an exception for an invalid pais', async () => {
    ...
    pais = {
      ...pais, nombre: "New name", descripcion: "New description"
    }
    await expect(() => service.update("0", pais)).rejects.toHaveProperty("message", "The pais with the given id was not found")
  }); 
  ...
  it('delete should throw an exception for an invalid pais', async () => {
    const pais: PaisEntity = paisesList[0];
    await service.delete(pais.id);
    await expect(() => service.delete("0")).rejects.toHaveProperty("message", "The pais with the given id was not found")
  });
