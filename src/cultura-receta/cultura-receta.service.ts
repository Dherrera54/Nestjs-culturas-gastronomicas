...
export class CulturaRecetaService {
 constructor(
    ...
    async addRecetaCultura(culturaId: string, recetaId: string): Promise<CulturaEntity> {
       ...
       const cultura: CulturaEntity = await this.culturaRepository.findOne({where: {id: culturaId}, relations: ["recetas"]})
       if (!cultura)
         throw new BusinessLogicException("La cultura gastronomica con id dado no se encontró", BusinessError.NOT_FOUND);
      ...
      async findRecetaByCulturaIdRecetaId(culturaId: string, recetaId: string): Promise<RecetaEntity> {
       ...
       if (!culturaReceta)
         throw new BusinessLogicException("La receta con id dado no esta asociada con la cultura", BusinessError.PRECONDITION_FAILED)
   ...
   async associateRecetasCultura(culturaId: string, recetas: RecetaEntity[]): Promise<CulturaEntity> {
       const cultura: CulturaEntity = await this.culturaRepository.findOne({where: {id: culturaId}, relations: ["recetas"]});
