...
it('addRecetaCultura should add an receta to a cultura', async () => {
   ...
   const result: CulturaEntity = await service.addRecetaCultura(newCultura.id, newReceta.id);
 ...
 it('addRecetaCultura should thrown exception for an invalid receta', async () => {
   ...
   await expect(() => service.addRecetaCultura(newCultura.id, "0")).rejects.toHaveProperty("message", "La receta con id dado no se encontró");
 });
 ...
 it('addRecetaCultura should throw an exception for an invalid cultura', async () => {
   ...
   await expect(() => service.addRecetaCultura("0", newReceta.id)).rejects.toHaveProperty("message", "La cultura gastronomica con id dado no se encontró");
 });

  it('findRecetaByCulturaIdRecetaId should return receta by cultura', async () => {
   const receta: RecetaEntity = recetasList[0];
   const storedReceta: RecetaEntity = await service.findRecetaByCulturaIdRecetaId(cultura.id, receta.id, )
 ...
 it('findRecetaByCulturaIdRecetaId should throw an exception for an invalid receta', async () => {
   await expect(()=> service.findRecetaByCulturaIdRecetaId(cultura.id, "0")).rejects.toHaveProperty("message", "La receta con id dado no se encontró");
 });

 it('findRecetaByCulturaIdRecetaId should throw an exception for an invalid cultura', async () => {
   const receta: RecetaEntity = recetasList[0];
   await expect(()=> service.findRecetaByCulturaIdRecetaId("0", receta.id)).rejects.toHaveProperty("message", "La cultura gastronomica con id dado no se encontró");
 });
 ...
 it('findRecetaByCulturaIdRecetaId should throw an exception for an receta not associated to the cultura', async () => {
   ...
   await expect(()=> service.findRecetaByCulturaIdRecetaId(cultura.id, newReceta.id)).rejects.toHaveProperty("message", "La receta con id dado no esta asociada con la cultura");
 });
 ...
 it('findRecetasByCulturaId should throw an exception for an invalid cultura', async () => {
   await expect(()=> service.findRecetasByCulturaId("0")).rejects.toHaveProperty("message", "La cultura gastronomica con id dado no se encontró");
 });
 ...
 it('associateRecetasCultura should throw an exception for an invalid cultura', async () => {
  ...
  nombre: faker.company.name(), 
   ...
   await expect(()=> service.associateRecetasCultura("0", [newReceta])).rejects.toHaveProperty("message", "La cultura gastronomica con id dado no se encontró");
 });
 ...
 it('associateRecetasCultura should throw an exception for an invalid receta', async () => {
   ...
   await expect(()=> service.associateRecetasCultura(cultura.id, [newReceta])).rejects.toHaveProperty("message", "La receta con id dado no se encontró");
 });
  ...
  it('deleteRecetaToCultura should thrown an exception for an invalid receta', async () => {
    await expect(()=> service.deleteRecetaCultura(cultura.id, "0")).rejects.toHaveProperty("message", "La receta con id dado no se encontró"); 
  });

  it('deleteRecetaToCultura should thrown an exception for an invalid cultura', async () => {
    const receta: RecetaEntity = recetasList[0];
    await expect(()=> service.deleteRecetaCultura("0", receta.id)).rejects.toHaveProperty("message", "La cultura gastronomica con id dado no se encontró"); 
  });
  ...
  it('deleteRecetaToCultura should thrown an exception for an non asocciated receta', async () => {
    ...
    await expect(()=> service.deleteRecetaCultura(cultura.id, newReceta.id)).rejects.toHaveProperty("message", "La receta con id dado no esta asociada con la cultura"); 
  }); 
