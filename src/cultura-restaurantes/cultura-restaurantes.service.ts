...
export class CulturaRestaurantesService {
    ...
    async addRestauranteCultura(culturaId: string, restauranteId: string): Promise<CulturaEntity> {
        ...
        const cultura: CulturaEntity = await this.culturaRepository.findOne({where: {id: culturaId}, relations: ["recetas", "restaurantes", "paises"]}) 
        if (!cultura)
          throw new BusinessLogicException("La cultura con el id dado no fue encontrada", BusinessError.NOT_FOUND);
    ...
    async findRestauranteByCulturaIdRestauranteId(culturaId: string, restauranteId: string): Promise<RestauranteEntity> {
        ...
        const cultura: CulturaEntity = await this.culturaRepository.findOne({where: {id: culturaId}, relations: ["restaurantes"]}); 
        if (!cultura)
          throw new BusinessLogicException("La cultura con el id dado no fue encontrada", BusinessError.NOT_FOUND)
    ...
    async associateRestaurantesCultura(culturaId: string, restaurantes: RestauranteEntity[]): Promise<CulturaEntity> {
        const cultura: CulturaEntity = await this.culturaRepository.findOne({where: {id: culturaId}, relations: ["restaurantes"]});
